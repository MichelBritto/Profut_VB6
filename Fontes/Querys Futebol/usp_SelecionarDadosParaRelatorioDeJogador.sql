IF(SELECT COUNT(*) FROM sysobjects WHERE xtype = 'P' AND name = 'usp_SelecionarDadosParaRelatorioDeJogador') >0
BEGIN
...
AS 
	...
	WHERE	JOG.NOMEATLETA_VC LIKE ISNULL(@Nome_VC,JOG.NOMEATLETA_VC) AND
			...
			JOG.EQUIPE_IN IN (SELECT EQUIPE_IN  FROM dbo.fnc_MontaEquipes(@Equipes_VC))
