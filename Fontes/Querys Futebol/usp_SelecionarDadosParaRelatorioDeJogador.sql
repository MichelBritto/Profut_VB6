...
AS 
    ...
    WHERE   JOG.NOMEATLETA_VC LIKE ISNULL(@Nome_VC, JOG.NOMEATLETA_VC)
            AND JOG.APELIDO_VC LIKE ISNULL(@Apelido_VC, JOG.APELIDO_VC)
            AND JOG.CARTEGORIA_IN = ISNULL(@Cartegoria_IN, JOG.CARTEGORIA_IN)
            AND JOG.DATANASCIMENTO_DT BETWEEN @DataNascimentoDE_DT AND @DataNascimentoATE_DT
            AND JOG.ESTADO_IN = ISNULL(@UF_IN, JOG.ESTADO_IN)
            AND JOG.CIDADE_VC LIKE ISNULL(@Cidade_VC, JOG.CIDADE_VC)
...
GRANT EXECUTE ON usp_SelecionarDadosParaRelatorioDeJogador TO PUBLIC